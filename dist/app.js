...
function itemThumb(item){const prod=PRODUCTS.find(x=>x.id===item.id),ph=prod&&photosOf(prod);return `<div class="cart-thumb">${item.preview&&validImageURL(item.preview)?`<img src="${item.preview}" alt="${esc(item.name)}">`:ph?photoPicture(ph[0],esc(item.name),'88px'):teePicture(item.base,esc(item.name),'88px')+graphicHTML(item)}</div>`;}
