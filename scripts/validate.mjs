...
for(const f of ['dist/vendor/motion.js','dist/vendor/three.js','dist/assets/tee.glb'])await access(f);
for(const file of ['dist/app.js','dist/commerce.js','dist/motion-ui.js','dist/studio-3d.js','dist/frame-guard.js','server.mjs'])execFileSync(process.execPath,['--check',file]);
...
const SITE='https://doavesso.vercel.app/';
...
for(const m of html.matchAll(/(?:href|content)="(https:\/\/doavesso\.vercel\.app\/[^"#]+)"/g))await access(`dist/${m[1].slice(SITE.length)}`);
