...
test('catalog filters and product selection feed the same persisted cart',async()=>{
 ...
 const stored=JSON.parse(s.w.localStorage.getItem('doavesso.cart.v1'));assert.equal(stored[0].size,'G');assert.equal(stored[0].price,8990);
...
test('checkout confirms a simulated order and never persists contact or address',async()=>{
 ...
 const raw=s.w.localStorage.getItem('doavesso.orders.v1'),order=JSON.parse(raw)[0];assert.equal(order.total,13480);assert.equal(order.shipping,'express');assert.equal(order.items[0].size,'M');assert(!raw.includes('cliente@example.com'));assert(!raw.includes('Rua de Exemplo'));assert(!raw.includes('Cliente de Exemplo'));
...
test('customized variant includes snapshot and survives cart reload',async()=>{
 ...
 saved=JSON.parse(s.w.localStorage.getItem('doavesso.cart.v1'));assert.equal(saved[0].size,'GG');assert.equal(saved[0].design.text,'MINHA IDEIA');assert(saved[0].preview.startsWith('data:image/jpeg'));assert.equal(saved[0].price,12990);
 }finally{s.close();}
 const r=await setup({'doavesso.cart.v1':saved});try{r.click('#open-cart');assert.equal(r.doc.querySelector('#cart-count').textContent,'1');assert(r.doc.querySelector('.cart-thumb img').src.startsWith('data:image/jpeg'));}finally{r.close();}
});
test('described print requires a brief, prices the design service and keeps the brief through reload',async()=>{
 ...
 s.doc.querySelector('#design-brief').value='curta';await submit();assert.equal(s.w.localStorage.getItem('doavesso.cart.v1'),null);
 ...
 saved=JSON.parse(s.w.localStorage.getItem('doavesso.cart.v1'));assert.equal(saved[0].price,14990);assert.equal(saved[0].design.mode,'brief');assert(saved[0].design.brief.includes('onda azul'));assert.equal(saved[0].design.image,undefined);
 ...
 const r=await setup({'doavesso.cart.v1':saved});try{r.click('#open-cart');assert.equal(r.doc.querySelector('.cart-item h3').textContent,'Sua camiseta · Estampa sob medida');assert(r.doc.querySelector('.brief-excerpt').textContent.includes('onda azul'));}finally{r.close();}
});
test('imperative tools register with schemas and reject invalid writes without changing cart',async()=>{
 const s=await setup();try{
 assert.equal(s.registry.size,2);const read=s.registry.get('read_doavesso_catalog_and_cart'),add=s.registry.get('add_doavesso_catalog_item_to_cart');
...
test('with the API online the catalog comes from the server and checkout submits an order without client prices',async()=>{
 ...
 const raw=s.w.localStorage.getItem('doavesso.orders.v1');assert(raw.includes('AV-TEST-0001'));assert(!raw.includes('cliente@example.com'));
...
test('accounts: signup asks for confirmation, login updates header, account lists orders, checkout prefills, logout clears',async()=>{
 ...
 assert(s.doc.querySelector('#auth-status').textContent.includes('confirma'));assert.equal(s.w.localStorage.getItem('doavesso.session.v1'),null);
 ...
 assert(JSON.parse(s.w.localStorage.getItem('doavesso.session.v1')).access_token==='tok');
 ...
 assert.equal(s.doc.querySelector('#open-auth').hidden,false);assert.equal(s.w.localStorage.getItem('doavesso.session.v1'),null);
 }finally{s.close();}
});
