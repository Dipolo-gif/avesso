...
- 14 testes passaram: contas (cadastro pede confirmação, erro de senha mapeado, login atualiza cabeçalho, “Minha conta” lista pedidos do banco com JWT, checkout pré-preenchido, logout limpa sessão); cálculos em centavos; frete no limite de gratuidade; quantidades e tamanhos; restauração de itens e preços; URLs de imagens e escape; designs adulterados no armazenamento são saneados; filtro e seleção de produto; sacola; checkout local sem persistência de contato; arte personalizada e restauração; estampa descrita (briefing obrigatório, preço próprio, persistência); modo online com `fetch` simulado (catálogo do servidor, pedido sem preços do cliente, consulta de status); contrato das ferramentas opcionais.
